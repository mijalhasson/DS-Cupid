...
def get_matched_rooms(df1: pd.DataFrame, df2: pd.DataFrame, output_columns: list[str]=[], room_col: str="processed_room_name", similarity_threshold: int=80):
    ...
    subset2 = subset2.dropna(subset=['matched_room'])  # Remove rows that did not meet threshold
...
def reference_room_match(lp_id: str, df1: pd.DataFrame, df2: pd.DataFrame, room_col: str="processed_room_name", similarity_threshold: int=80):
    """
    ...
    df_matched = get_matched_rooms(subset1, df2, output_columns=['supplier_room_id', 'supplier_room_name'])
...
def match_hotel_rooms(rooms_data: RoomData):
    ...
    df_reference['processed_room_name'] = normalize_room_name(df_reference["reference_room_name"])
    df_input['processed_room_name'] = normalize_room_name(df_input["supplier_room_name"])
    df_matched, df_reference_unmatched, df_input_unmatched = map_rooms(df_reference, df_input)
    ...
    for reference_room_id, group in grouped:
        mapped_rooms = group[["supplier_name", "supplier_room_id", "supplier_room_name", "match_score"]].to_dict(orient='records')
        reference_details = {
